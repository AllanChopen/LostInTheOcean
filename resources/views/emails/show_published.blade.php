          ...
          <table role="presentation" cellpadding="0" cellspacing="0" style="max-width:640px;width:100%;background:#0f1113;border-radius:10px;overflow:hidden;box-shadow:0 14px 40px rgba(2,6,23,0.6);border:1px solid rgba(255,255,255,0.03);">
            <tr>
              <td style="padding:28px 24px;text-align:center;background:linear-gradient(180deg,#0b0b0d 0%,#2b1b26 100%);">
                ...
                <div style="font-family: Georgia, 'Times New Roman', serif; color:#d9caa7; letter-spacing:1px; font-size:16px; font-weight:600;">Anuncio de show</div>
              </td>
            </tr>
            <tr>
              <td style="padding:24px 28px;">
                <h2 style="margin:0 0 10px;font-size:20px;color:#f3efe6;font-family: Georgia, 'Times New Roman', serif;">{{ $show->title }}</h2>
                @if(!empty($show->poster_image))
                  <div style="margin:14px 0;text-align:center;"><img src="{{ $show->poster_image }}" alt="{{ $show->title }}" style="max-width:100%;height:auto;border-radius:6px;display:inline-block;border:1px solid rgba(255,255,255,0.03);"/></div>
                @endif
                <p style="margin:0 0 14px;color:#cfc9c2;line-height:1.6;">{{ \Illuminate\Support\Str::limit(strip_tags($show->description), 300) }}</p>

                <div style="margin:18px 0;text-align:center;">
                  <a href="{{ $url }}" style="display:inline-block;padding:10px 14px;background:#bfa46a;color:#0b0b0d;border-radius:6px;text-decoration:none;font-weight:700;">Ver detalles</a>
                </div>
